...
Ter achtergrondinformatie: de steekproef van meting 1 was een aselecte steekproef van LISS-panelleden van 16 jaar en ouder die ook de LISS-kernvragenlijst ‘Health’ (afgenomen in november en december 2019) en de vragenlijst over het coronavirus (afgenomen in het LISS panel maart 2020) hebben ingevuld.  
...
```{r, echo=FALSE, fig.width=5.5, fig.height=4, dpi = 300, fig.cap="Waargenomen ernst en vatbaarheid over de tijd, totaal percentage dat minstens een beetje eens was met de stellingen (een beetje mee eens, mee eens, helemaal mee eens)."}
...
```{r, echo=FALSE, fig.width=5.5, fig.height=2.5, dpi = 300, fig.cap="Naleving algemene gedragsregels bij klachten, totaal percentage dat vaak, meestal, of altijd naar buiten is geweest met klachten."}
...
```{r McNemarBI1aUtautovertime, echo=FALSE}
# First calculate a proportion for the variable for the current wave. 
...
# Convert into a table that can be used for the chi square test
mcnemar.table = table(data_currentwave_wavemin1$BI1a_UTAUT_agree_cw, data_currentwave_wavemin1$BI1a_UTAUT_agree_wmin1)
mcnemartest = mcnemar.test(mcnemar.table)
```
De McNemar toets (X²(`r mcnemartest$parameter`) = `r round(mcnemartest$statistic, digits = 2)`, p `r ifelse(mcnemartest$p.value < 0.001, "<0.001", paste("=", round(mcnemartest$p.value, digits = 3)))`) liet zien dat er `r if(mcnemartest$p.value > 0.05){"g"}`een significant verschil was tussen de huidige meting (`r round((nrow(filter(data_currentwave_wavemin1, (BI1a_UTAUT_agree_cw == 'Totaal eens_cw'))) /nrow(filter(data_currentwave_wavemin1)))*100, digits = 1)`%) en de vorige meting (`r round((nrow(filter(data_currentwave_wavemin1, (BI1a_UTAUT_agree_wmin1 == 'Totaal eens_wmin1'))) /nrow(filter(data_currentwave_wavemin1)))*100, digits = 1)`%) in het percentage dat aangaf van plan te zijn de CoronaMelder in de komende 2 maanden te gebruiken.
```{r McNemarBI2aUtautovertime, echo=FALSE}
# First calculate a proportion for the variable for the current wave. 
data_currentwave_wavemin1$BI2a_UTAUT_agree_cw[(data_currentwave_wavemin1$BI2a_UTAUT_cw == 'Helemaal mee eens'| data_currentwave_wavemin1$BI2a_UTAUT_cw == 'Mee eens'| data_currentwave_wavemin1$BI2a_UTAUT_cw == 'Een beetje mee eens')] <- "Totaal eens_cw"
data_currentwave_wavemin1$BI2a_UTAUT_agree_cw[(data_currentwave_wavemin1$BI2a_UTAUT_cw == 'Neutraal'| data_currentwave_wavemin1$BI2a_UTAUT_cw == 'Een beetje oneens'| data_currentwave_wavemin1$BI2a_UTAUT_cw == 'Oneens'| data_currentwave_wavemin1$BI2a_UTAUT_cw == 'Helemaal mee oneens')] <- "Neutraal, niet mee eens_cw"

# Calculate a proportion for the variable for the previous wave. 
data_currentwave_wavemin1$BI2a_UTAUT_agree_wmin1[(data_currentwave_wavemin1$BI2a_UTAUT_wmin1 == 'Helemaal mee eens'| data_currentwave_wavemin1$BI2a_UTAUT_wmin1 == 'Mee eens'| data_currentwave_wavemin1$BI2a_UTAUT_wmin1 == 'Een beetje mee eens')] <- "Totaal eens_wmin1"
data_currentwave_wavemin1$BI2a_UTAUT_agree_wmin1[(data_currentwave_wavemin1$BI2a_UTAUT_wmin1 == 'Neutraal'| data_currentwave_wavemin1$BI2a_UTAUT_wmin1 == 'Een beetje oneens'| data_currentwave_wavemin1$BI2a_UTAUT_wmin1 == 'Oneens'| data_currentwave_wavemin1$BI2a_UTAUT_wmin1 == 'Helemaal mee oneens')] <- "Neutraal, niet mee eens_wmin1"

# Convert into a table that can be used for the chi square test
mcnemar.table = table(data_currentwave_wavemin1$BI2a_UTAUT_agree_cw, data_currentwave_wavemin1$BI2a_UTAUT_agree_wmin1)
mcnemartest = mcnemar.test(mcnemar.table)
```
De McNemar toets (X²(`r mcnemartest$parameter`) = `r round(mcnemartest$statistic, digits = 2)`, p `r ifelse(mcnemartest$p.value < 0.001, "<0.001", paste("=", round(mcnemartest$p.value, digits = 3)))`) liet zien dat er `r if(mcnemartest$p.value > 0.05){"g"}`een significant verschil was tussen de huidige meting (`r round((nrow(filter(data_currentwave_wavemin1, (BI2a_UTAUT_agree_cw == 'Totaal eens_cw'))) /nrow(filter(data_currentwave_wavemin1)))*100, digits = 1)`%) en de vorige meting (`r round((nrow(filter(data_currentwave_wavemin1, (BI2a_UTAUT_agree_wmin1 == 'Totaal eens_wmin1'))) /nrow(filter(data_currentwave_wavemin1)))*100, digits = 1)`%) in het percentage dat aangaf het eens te zijn met de stelling dat het waarschijnlijk is dat men de CoronaMelder in de komende 2 maanden zal gebruiken.
...
#### Verwachte gebruiksvriendelijkheid, inspanningsverwachting en zelfeffectiviteit over de tijd
